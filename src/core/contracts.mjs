...
function validateButtonContract(c) {
  const required = ["name", "version", "element", "props", "slots", "a11y"];
  for (const k of required) {
    if (!(k in c)) throw new Error(`Invalid contract: missing '${k}'`);
  }
  if (c.name !== "Button") throw new Error(`Invalid contract: expected name 'Button'`);
  if (c.element !== "button") throw new Error(`Invalid contract: expected element 'button'`);
  return c;
}

export function listButtonVersions() {
  const dir = path.join(CONTRACTS_ROOT, "button");
  return fs.readdirSync(dir).filter(f => f.endsWith(".json")).map(f => f.replace(".json","")).sort();
}

export function loadButtonContract(version) {
